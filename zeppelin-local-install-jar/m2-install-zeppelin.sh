...
cd $ZEPPELIN_DIR
mvn clean package -Pbuild-distr -Pspark-2.0 -Phadoop-2.4 -Pyarn -Ppyspark -Psparkr
#scp zeppelin-distribution/target/zeppelin-${version}.tar.gz glassfish@snurran.sics.se:/var/www/hops
scp zeppelin-distribution/target/zeppelin-${version}-bin-spark-2.0_hadoop-2.4.tar.gz glassfish@snurran.sics.se:/var/www/hops
